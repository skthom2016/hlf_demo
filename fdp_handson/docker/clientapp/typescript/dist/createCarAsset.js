...
async function main() {
    try {
        ...
        await lib_1.changePoStatus(po_contract, 'acceptedPo', poId, po);
        ...
        await lib_1.readCarsAutoAssigned(car_contract, poId, po);
...
// 
async function createCarAssets(car_contract, item, PoSl) {
    ...
    try {
        ...
        await lib_1.prepareSoLineVhid(car_contract, PoSl, VhidArr);
    }
    catch (error) {
        console.log(error);
...
async function CarsubmitTxnContract(car_contract, Vhid, orderJson) {
    try {
        // Submit the transaction
...
//# sourceMappingURL=createCarAsset.js.map